...
//        if non-existing function -> send it to new_func array DONE
...
// List of functions to be made overloadable
old_func = [...
'diff';...
'eval';...
'gcd';...
...
clear units typs
for k=size(nams,'*'):-1:1
  l=strindex(nams(k),'updaternew.sce');
  if l<>[] then
...
for ii = 1:maxi(size(old_func));
   ...
   if ~isempty(path_and_fichier);
      ...
      // Function which are not library functions
      isbuitin = %f;
      execstr('if type('+old_func(ii)+')==130; isbuitin = %t; end;','errcatch');
      if isbuitin;
          write(%io(2),'### WARNING: '+old_func(ii)+' cannot be overloaded','(a)');
      else
         new_func(ll) = old_func(ii);
         ll = ll + 1;
      end;
   end;
end;
